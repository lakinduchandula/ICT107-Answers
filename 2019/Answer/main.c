// this is the answer for B.Sc. (General) Degree First Year First Semester
// Terminal Course Unit Examination - June 2019
#include <stdio.h>
...
int rooms[40] = {0}; // initialize all rooms for 0 (available)

// function main begins program execution
int main() {
  while (1) { // enter to infinite loop
    int userResponse;
...
void spendDayPackage() {
  ...
  // check Spend the day package is offered to a group of people with more than
  // five members
  if (noKids + noAdults < 5) {
    printf("This package is offered to a group of people with more than five "
           "members\n\n");
    return;
  }
  ...
  totalAmount = ADULT_SPEND_DAY_PKG * noAdults + KID_SPEND_DAY_PKG * noKids +
                POOL_GYM_SPEND_DAY_PKG * usePoolGym * noAdults;
...
void calculateBill() {
  ...
  printf("Package Details : %s room with %s Board for %d Nights.\n",
         roomType <= 2 ? "Double" : "Triple",
         roomType == 1 || roomType == 3 ? "Half" : "Full", noNights);
  printf("Total Amount    : %.2f\n", finalAmount);
}
...
// Complete Answer - 20-11-2022 Last Updated